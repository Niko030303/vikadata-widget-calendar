...
export const Setting: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 这里记得加个日期格式的转换
    const newData<Array></Array> = []
    for (const record of records) {
      newData.push(record.getCellValueString(fieldId)?.slice(0, 10))
    }

    // const newData = records.forEach((record) => {
    //   // const demo = record?.getCellValue(fieldId) as number
    //   // console.log('demo', demo )
    //   // const result =  DateTime.fromMillis(record?.getCellValue(fieldId) as number).toFormat('yyyy-MM-dd')
    //   // console.log(result)
    //   // return result
