...
export const Setting: React.FC = () => {
  ...
  const [data, setData] = useCloudStorage<any>('setCalendarData', [])
  ...
  useEffect(() => {
    ...
    // const newData = records.forEach((record) => {
    //   // const demo = record?.getCellValue(fieldId) as number
    //   // console.log('demo', demo )
    //   const result =  DateTime.fromISO(record?.getCellValueString(fieldId) as string).toFormat('yyyy-MM-dd')
    //   console.log(result)
    //   return result
    //   // return record.getCellValueString(fieldId)?.slice(0, 10)
    // })
    //   // return DateTime.fromFormat(record?.getCellValueString(fieldId)?.slice(0, 10) as string, 'yyyy-MM-dd')[ts]
    //   // return record.getCellValueString(fieldId)?.slice(0, 10)
      
    // })
    const newData = records.map((record) => {
      // return DateTime.fromMillis(record?.getCellValue(fieldId) as number).toFormat('yyyy-MM-dd')
      // return DateTime.fromFormat(record?.getCellValueString(fieldId)?.slice(0, 10) as string, 'yyyy-MM-dd')[ts]
      return record.getCellValueString(fieldId)?.slice(0, 10)
      DateTime.fromISO(record?.getCellValueString(fieldId) as string).toFormat('yyyy-MM-dd')
