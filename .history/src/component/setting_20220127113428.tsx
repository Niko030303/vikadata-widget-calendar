...
export const Setting: React.FC = () => {
  ...
  useEffect(() => {
    ...
    //   return DateTime.fromMillis(Date.parse(record.getCellValueString(fieldId)) as number)
   ...
   let newData = []
    records.forEach((record) => {
      // return record.getCellValueString(fieldId)?.slice(0, 10)
      const result = DateTime.fromMillis(Date.parse(record.getCellValueString(fieldId)?.slice(0, 10)) as number).toUTC()
      if (result.toFormat('yyyy-MM-dd') === 'Invalid Time')
      newData.push(result)
      // return DateTime.fromISO(record?.getCellValueString(fieldId) as string).toFormat('yyyy-MM-dd')
    })
